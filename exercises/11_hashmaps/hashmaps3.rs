...
/*
        let team_1: Team = scores.get(&team_1_name).map(|value| {
            Team {goals_conceded: value.goals_conceded, goals_scored: value.goals_scored}
        }).unwrap_or_else(|| {
            Team { goals_conceded: 0, goals_scored: 0 }
        });

        let team_2: Team = scores.get(&team_2_name).map(|value| {
            Team {goals_conceded: value.goals_conceded, goals_scored: value.goals_scored}
        }).unwrap_or_else(|| {
            Team { goals_conceded: 0, goals_scored: 0 }
        });
